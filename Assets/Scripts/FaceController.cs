...
public class FaceController : MonoBehaviour
{
  ...
  private void Awake()
  {
    ...
    dictCategoricalEmotions.Add("Neutral", new List<int>()); // Empty list for neutral expressi
...
public void setCategoricalEmotion(string strEmotionName, float fWeight) {
    ...
    // ADD DEBUG LOG HERE
    Debug.Log($"setCategoricalEmotion called: {strEmotionName}, weight: {fWeight}");

    // resetAUs();
    if (dictCategoricalEmotions.ContainsKey(strEmotionName)) {
        // ADD DEBUG LOG HERE
        Debug.Log($"Emotion found in dictionary: {strEmotionName}");
        List<int> lstAUs = dictCategoricalEmotions[strEmotionName];
        foreach (int AU in lstAUs) {
            // ADD DEBUG LOG HERE
            Debug.Log($"Setting AU: {AU} to weight: {fWeight}");
            setAU(AU.ToString(), "both", fWeight);
        }
    } else {
        // ADD DEBUG LOG HERE - This is crucial!
        Debug.LogError($"Emotion NOT found in dictionary: {strEmotionName}. Check spelling?");
    }
}
